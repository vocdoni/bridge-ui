...
import { useRegisteredTokens } from "./registered-tokens";
...
export function UseUserTokens({ children }) {
    ...
    const refreshUserTokens = async (tokenAddresses: string[]): Promise<TokenBalance[]> => {
        ...
        const contractCalls = tokenAddresses.map(address => (new Contract(address, ERC20JsonAbi)).balanceOf(wallet.account))
