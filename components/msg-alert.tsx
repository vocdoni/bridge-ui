...
export const MessageAlert = () => {
  const { message, clearAlert, error } = useMessageAlert();

  return (
    <AlertContainer visible={!!message?.length} error={error}>
      <AlertText error={error}>{message || ""}</AlertText>
      <CloseAlert error={error} onClick={clearAlert}>
        <Cross error={error} />
      </CloseAlert>
    </AlertContainer>
  );
...
const CloseAlert = styled.div<{ error: boolean }>`
  stroke: ${({ theme, error }) => error ? theme.functionality.f3 : theme.functionality.f4};
  pointer: cursor;
  position: absolute;
  top: 16px;
  right: 16px;
...
const AlertText = styled.div<{ error: boolean }>`
  color: ${({ theme, error }) => error ? theme.functionality.f3 : theme.functionality.f4};
  margin-right: 49px; 
  font-size: 16px;
  text-align: left;
`;

const AlertContainer = styled.div<{ visible: boolean, error: boolean }>`
  position: fixed;
  z-index: 320;
  bottom: 90px;
  right: 16px;
  ...
  background-color: ${({ error }) => error ? "#F8E8EB" : "#DEF4E8"};
  border-radius: 10px;
